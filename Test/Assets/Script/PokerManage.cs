...
public class PokerManage : MonoBehaviour
{
    ...
    void Start()
    {
        mPlayer1 = GameObject.FindWithTag("player1");
        mPlayer2 = GameObject.FindWithTag("player2");
        mPlayer3 = GameObject.FindWithTag("player3");
    ...
    public void OutPoker(ArrayList pokers)
    {
        mIsNext = false;
        mPlayer1.GetComponent<PokerManage>().ClearOutPoker();
        mPlayer2.GetComponent<PokerManage>().ClearOutPoker();
        mPlayer3.GetComponent<PokerManage>().ClearOutPoker();
        pokers.Sort(new PokerSort());
        outPokers.AddRange(pokers);
        int length = pokers.Count;
        float first = center.x - (Math.Max(0, pokers.Count - 1) * interval * outPokerSclae) / 2;
        ...
        switch (gameObject.name)
        {
            case "Player1":
                ...
                {
                    RectTransform rt = (pokers[f] as GameObject).GetComponent<RectTransform>();
                    (pokers[f] as GameObject).GetComponent<Poker>().ClickAble(false);
            ...
            case "Player2":
                for (int f = 0; f < length; f++)
                {
                    RectTransform rt = (pokers[0] as GameObject).GetComponent<RectTransform>();
                    Vector3 ps2 = center -  new Vector2(first, center.y - outPokerDistance + f * interval * outPokerSclae);
                    rt.position = MathTool.Rotate(ps2, Vector3.forward, 90) + new Vector3(center.x, center.y, 0);
                    (pokers[0] as GameObject).GetComponent<Poker>().Turn();
                    (pokers[0] as GameObject).transform.Rotate(0, 0, -90);
                    System.Object o = pokers[0];
                    pokers.RemoveAt(0);
                    this.pokers.Remove(o);
                }
                break;
            case "Player3":
                pokers.Reverse();
                ...
                {
                    RectTransform rt = (pokers[0] as GameObject).GetComponent<RectTransform>();
                    Vector3 ps3 = center - new Vector2(first, center.y - outPokerDistance + f * interval * outPokerSclae);
                    rt.position = MathTool.Rotate(ps3, Vector3.forward, -90) + new Vector3(center.x, center.y, 0);
                    (pokers[0] as GameObject).GetComponent<Poker>().Turn();
                    (pokers[0] as GameObject).transform.Rotate(0, 0, -90);
                System.Object o = pokers[0];
                    pokers.RemoveAt(0);
    ...
    public void ClearOutPoker()
    {
        while(outPokers.Count != 0)
        {
            Destroy(outPokers[0] as GameObject);
            outPokers.RemoveAt(0);
        }
    }
